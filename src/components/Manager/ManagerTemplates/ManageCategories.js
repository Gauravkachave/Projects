...
const ManageCategories= (props) =>{
    const { classes ,selectFolder,inputs,handleChange} = props;
    const [editDialog, setEditDialog] = useState(false);
    const [expanded, setExpanded] = React.useState(false);
    const handleExpandClick = () => { setExpanded(!expanded) };
    // console.log(inputs.folder_id);
    return (
        <React.Fragment>
            <Span px={4}>
                ...
                <Hidden only="xs">
                    <TableContainer component={Paper}>
                        <Table className={classes.TableCss}>
                            ...
                            <TableBody>
                                <TableRow>
                                    <TableCell classes={{ root: classes.TableCellCss }}>New Consultant Series</TableCell>
                                    <TableCell classes={{ root: classes.TableCellEditCss }} onClick={() => { setEditDialog(!editDialog) }}>Edit</TableCell>
                                    {/* edit dialog */}
